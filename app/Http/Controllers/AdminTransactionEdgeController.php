...
class AdminTransactionEdgeController extends Controller
{
    public function adminEdgeDashboard(Request $request)
    {
    ...
    public function index(Request $request)
    {
        ...
        // Kalau BUKAN admin, filter berdasarkan user mapping
        if (!$isAdmin) {
            $query->whereHas('device', function ($q) use ($userName) {
        ...
        // Eksekusi query
        $transactionEdge = $query->orderBy('detection_time', 'desc')
            ->paginate($perPage)
            ->withQueryString();
