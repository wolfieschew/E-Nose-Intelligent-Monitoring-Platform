            ...
            <main class="h-full overflow-y-auto">
                <div class="container px-6 mx-auto grid">
                    ...
                    <div class="w-full overflow-hidden rounded-lg shadow-xs">
                            ...
                            <table id="transactionTable" class="w-full whitespace-no-wrap">
                                <thead>
                                    <tr
                                        ...
                                        <th class="px-4 py-3">Data Prediksi</th>
                                    ...
                                    @foreach ($data as $item)
                                        <tr id="tr" class="text-gray-700 dark:text-gray-400">
                                            ...
                                            <td class="px-4 py-3 text-sm">
                                                ...
                                                <button
                                                    onclick="downloadRowFromServer('{{ $item['date_time'] }}') "class="font-semibold">Download</button>
                                            </td>
                        ...
                        <div
                            class="grid px-4 py-3 text-xs font-semibold tracking-wide text--500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
...
<script>
    document.getElementById('searchInput').addEventListener('input', function () {
        ...
        rows.forEach(function (row) {
            ...
            cells.forEach((cell, index) => {
                ...
                if (searchValue && originalText.toLowerCase().includes(searchValue)) {
                    ...
                    const highlighted = originalText.replace(regex, `<span class=" text-red-600 font-semibold rounded px-1">$1</span>`);
        ...
        document.getElementById('resultCount').textContent = searchValue ? `Menampilkan ${count} hasil pencarian.` : '';
    });
</script>


    <script>
        function downloadRowFromServer(dateTime) {
            const encoded = encodeURIComponent(dateTime);
            // Redirect ke endpoint Laravel yang menangani CSV export
            window.location.href = `/download-row?date_time=${encoded}`;
        }
    </script>
</body>
